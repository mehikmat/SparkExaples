...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf().setAppName("Lang Ranking").setMaster("local")
   ...
   *   several seconds.
   */
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = index.mapValues(articles => articles.size).filter(_._2 != 0).collect().toList.sortBy(-_._2)
   ...
   *   several seconds.
   */
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = rdd.flatMap(article => langs.map(lang =>(lang,article.mentionsLanguage(lang) match { case true => 1 case false => 0}))).reduceByKey((a,b) => a+b).collect().filter(_._2 != 0).toList.sortBy(-_._2)
