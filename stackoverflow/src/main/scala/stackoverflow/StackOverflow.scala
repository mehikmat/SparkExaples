...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val questionRdd = postings.filter(post => post.postingType == 1).map(post => (post.id,post))
    val answerRdd = postings.filter(post => post.postingType == 2).map(post => (post.parentId.get,post))
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.map(kv => (kv._2.head._1, answerHighScore(kv._2.map(vv => vv._2).toArray)))
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map(kv => (firstLangInTag(kv._1.tags, langs).getOrElse(-1)*langSpread,kv._2)).filter(v => v._1 >= 0).cache()
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    val meanMap = vectors.map(point => (findClosest(point,means),point)).groupByKey().mapValues(v => averageVectors(v)).collect().toMap
    val newMeans = means.indices.map(i => meanMap.getOrElse(i,means(i))).toArray
