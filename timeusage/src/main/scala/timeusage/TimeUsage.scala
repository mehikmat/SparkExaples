...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def startFilter(col:String, filter: List[String]): Boolean = {
      filter.map(c => col.startsWith(c)).reduce(_||_)
    }

    val p = columnNames.filter(col => startFilter(col,primaryNeeds)).map(col => new Column(col))
    val w = columnNames.filter(col => startFilter(col,workingActivities)).map(col => new Column(col))
    val o = columnNames.filter(col => startFilter(col,otherActivities) && !startFilter(col,primaryNeeds) && !startFilter(col,workingActivities)).map(col => new Column(col))
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when($"telfs" < 3 and $"telfs" >= 1, "working").otherwise("not working").alias("working")
    ...
    val primaryNeedsProjection: Column = primaryNeedsColumns.map(c => c/60).reduce(_+_).alias("primaryNeeds")
    val workProjection: Column = workColumns.map(c => c/60).reduce(_+_).alias("work")
    val otherProjection: Column = otherColumns.map(c => c/60).reduce(_+_).alias("other")
    df
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(tu => (tu.working,tu.sex,tu.age))
      .agg(round(typed.sum[TimeUsageRow](_.primaryNeeds),1).as[Double], round(typed.sum[TimeUsageRow](_.work),1).as[Double], round(typed.sum[TimeUsageRow](_.other),1).as[Double])
